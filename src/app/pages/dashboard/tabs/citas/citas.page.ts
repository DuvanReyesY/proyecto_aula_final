...
generarSlots() {
  ...
  isSlotOcupado(slot: string): boolean {
    ...
    // En modo crear: bloquear todos los slots si la fecha es pasada
  ...
  onVeterinarioChange() {
    this.horasSeleccionadas = this.horasSeleccionadas.filter(s => !this.isSlotOcupado(s));
    if (this.horasSeleccionadas.length > 0) {
      const sorted = this.horasSeleccionadas
  ...
  onFechaChange() { this.onVeterinarioChange(); }
